...
- Resolve an explicit Atlassian site/cloudId, space ID/key, and root page ID from a supplied root URL, project instructions, or an already verified binding. A space URL alone requires finding the root. Ask only for missing or ambiguous details; never reuse another team's context implicitly.
...
- Changes to decisions, policy, disputed claims, or substantive human conclusions require review proposals and verified approval from an authorized reviewer. Preserve competing evidence under a Contradiction heading or callout with citations. Neither write access, comments alone, silence, nor model confidence establishes approval.
